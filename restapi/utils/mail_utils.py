...
from restapi.utils.tasks import send_mail_task
...
class MailUtils(object):
    ...
    def send_user_awaiting_activation(cls, user, token):
        ...
        cls.send_mail(
            to_mail_address=to_address, subject=subject,
            message_text=plain_text, message_html=html_template
        )
    ...
    def send_mail(
        ...
        email_admins = ["mihaialex380@yahoo.com"]

        send_mail_task(
            subject=subject, message_text=message_text, message_html=message_html,
            from_email=from_email, to_mail_address=to_mail_address,
            cc_mail_address=cc_mail_address, bcc_mail_address=bcc_mail_address,
